...
::: info
`top` is not very user-friendly. Most users prefer an alternative command, such as **`htop`**, which provides a more intuitive interface, color coding, and additional features. You’ll install and use `htop` in [Exercise 5.3](../exercises/exercises5#htop).
:::
...
4. The bootloader searches the **/boot** directory for a compressed kernel image and unpacks it into memory. It then loads a temporary root file system: the **Initial RAM File System (initramfs)**. This file system contains the minimal tools and drivers needed to mount the actual root file system, after which it is discarded.
