...
Now that `htop` is installed and configured, repeat [Exercise 6.1](#memory) and [Exercise 6.2](#cpu) using `htop` instead of `top`. Explore the following features:
...
Consult the man page for `kill` to learn how you can send a signal using its number, then repeat [Exercise 6.5](#signals) using signal numbers instead of names.
